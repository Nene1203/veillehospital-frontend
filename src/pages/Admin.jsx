...
function PasswordInput({ value, onChange, placeholder = "Mot de passe" }) {
...
export default function Admin() {
  ...
  const handleSave = async () => {
    ...
    if (isNew) {
      if (!editUser.mot_de_passe) { setError("Le mot de passe est obligatoire"); return; }
      if (editUser.mot_de_passe.length < 8) { setError("Le mot de passe doit contenir au moins 8 caractères"); return; }
      if (editUser.mot_de_passe !== confirmPassword) { setError("Les mots de passe ne correspondent pas"); return; }
    }
  ...
  const handleToggleActif = async (user) => {
    const res = await fetch(`${BASE_URL}/auth/users/${user.id}`, {
      method: "PATCH", headers, body: JSON.stringify({ actif: !user.actif }),
    });
  ...
  const toggleEtab = (etabId) => {
    setEditUser((prev) => {
      ...
      return { ...prev, etablissement_ids: ids.includes(etabId) ? ids.filter((id) => id !== etabId) : [...ids, etabId] };
    });
  };
  ...
  const passwordsMatch = confirmPassword === "" || editUser?.mot_de_passe === confirmPassword;
  const canSave = !saving && (editUser?.id || (passwordsMatch && editUser?.mot_de_passe?.length >= 8));
  ...
  return (
    <div style={{ padding: "32px", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: "#111827", margin: 0 }}>⚙️ Gestion des utilisateurs</h1>
          <p style={{ color: "#6b7280", marginTop: 6, fontSize: 14 }}>{users.length} utilisateur{users.length > 1 ? "s" : ""} au total</p>
        </div>
        <button onClick={openCreate} style={{ background: "#1a56db", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
          + Nouvel utilisateur
        </button>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
                ...
                <tr key={user.id} style={{ borderBottom: i < filtered.length - 1 ? "1px solid #f3f4f6" : "none" }}
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: roleInfo.bg, color: roleInfo.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <span style={{ background: roleInfo.bg, color: roleInfo.color, padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>{roleInfo.label}</span>
                  </td>
                  <td style={{ padding: "14px 16px", fontSize: 13, color: "#6b7280" }}>
                    {user.etablissements?.length > 0 ? user.etablissements.map((e) => e.nom).join(", ") : <span style={{ color: "#d1d5db" }}>—</span>}
                  </td>
                  <td style={{ padding: "14px 16px" }}>
                    <span style={{ background: user.actif ? "#f0fdf4" : "#fef2f2", color: user.actif ? "#059669" : "#dc2626", padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                      {user.actif ? "Actif" : "Inactif"}
                    </span>
                  </td>
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => openEdit(user)} style={{ background: "#f3f4f6", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>Modifier</button>
                      <button onClick={() => handleToggleActif(user)} style={{ background: user.actif ? "#fef2f2" : "#f0fdf4", color: user.actif ? "#dc2626" : "#059669", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={6} style={{ padding: 40, textAlign: "center", color: "#9ca3af", fontSize: 14 }}>Aucun utilisateur trouvé</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showModal && editUser && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto", padding: 32 }}>
            ...
            {!editUser.id && (
              <>
                <div style={{ marginBottom: 16 }}>
                  ...
                  />
                  {editUser.mot_de_passe && editUser.mot_de_passe.length < 8 && (
                    <div style={{ marginTop: 4, fontSize: 12, color: "#f59e0b" }}>⚠️ Minimum 8 caractères</div>
                  )}
                </div>
                <div style={{ marginBottom: 16 }}>
                  ...
                  />
                  {confirmPassword && (
                    <div style={{ marginTop: 4, fontSize: 12, color: passwordsMatch ? "#059669" : "#dc2626" }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, maxHeight: 160, overflowY: "auto" }}>
                  {etablissements.map((etab) => (
                    <label key={etab.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #f3f4f6", fontSize: 14 }}>
                      <input type="checkbox" checked={(editUser.etablissement_ids || []).includes(etab.id)} onChange={() => toggleEtab(etab.id)} />
                      {etab.nom}
                    </label>
                  ))}
                </div>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
              <button onClick={() => setShowModal(false)} style={{ background: "#f3f4f6", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 14, cursor: "pointer", fontWeight: 500 }}>
                Annuler
              </button>
              <button onClick={handleSave} disabled={!canSave} style={{ background: "#1a56db", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 14, cursor: canSave ? "pointer" : "not-allowed", fontWeight: 600, opacity: canSave ? 1 : 0.5 }}>
